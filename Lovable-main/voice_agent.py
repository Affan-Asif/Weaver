...
It is a small, *separate* FastAPI service that REUSES the builder's engine:
  - generation:  llm.py + prompts.py     (Anthropic Claude)
  - storage:     storage.py              (shares config.json + projects with the UI)
  - deploy:      deploy.py               (Vercel REST API)
  - telephony + WhatsApp: Twilio

Because it reuses storage.config.json, your Anthropic key and Vercel token are read
from the SAME place you set them in the builder's ⚙ Settings. Twilio + WhatsApp
settings come from a local .env (see .env.example).
...
def generate_site(description: str) -> tuple[dict, str, str]:
    ...
    if not key:
        raise RuntimeError("No Anthropic API key — set it in the builder's Settings first.")

    # Voice transcripts can be terse or cut off mid-sentence ("I would like you
    # to build"). Nudge Claude to commit to a reasonable interpretation and the
    # required file-block format instead of asking the caller a clarifying question.
    voice_nudge = (
        "\n\nNote: this request came from a phone call, so the transcription may be "
        "short, vague, or cut off. Do NOT ask clarifying questions. Pick the most "
        "reasonable interpretation and build a complete, polished website. Output "
        "ONLY the file blocks in the exact OUTPUT FORMAT — nothing before or after."
    )
    user_prompt = prompts.build_generate_user_prompt(description, has_image=False) + voice_nudge
    ...
    try:
        ...
        # One retry with an even more directive prompt — sometimes the model
        # responds conversationally on the first pass despite the system rules.
        retry_prompt = (
            user_prompt
            + "\n\nYour previous reply did not contain any <<<FILE: ... >>> blocks. "
            "Reply again with ONLY the file blocks, starting with index.html."
        )
...
<h3>Readiness</h3>
<div>
{chip(bool(cfg.get('anthropic_api_key')), 'Anthropic key')}
{chip(bool(cfg.get('vercel_token')), 'Vercel token')}
{chip(bool(TWILIO_SID and TWILIO_AUTH), 'Twilio creds')}
{chip(bool(WHATSAPP_TO), 'WhatsApp recipient')}
</div>
<p style="color:#555;font-size:14px">Set Anthropic &amp; Vercel in the builder's ⚙ Settings.
Set Twilio &amp; WhatsApp in <code>.env</code>. Point your Twilio number's Voice webhook to
<code>/voice/incoming</code> (POST).</p>
<h3>Recent jobs</h3>
<table border=1 cellpadding=6 style="border-collapse:collapse;width:100%;font-size:14px">

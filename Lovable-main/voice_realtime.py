...
#             print(f"❌ Deepgram websocket failed: {e}")
#             raise
...
#                 try:
#                     if sess.dg_ws and not sess.dg_ws.closed:
#                         await sess.dg_ws.send(audio)
#                 except Exception as e:
#                     print(f"❌ Failed sending audio to Deepgram: {e}")
...
# Windows consoles default to cp1252, which cannot encode the emoji used in our
# log prints (🟢 ❌ 🎤 …). Without this, the first such print raises
# UnicodeEncodeError inside the Twilio WebSocket handler and the call drops the
# instant the caller picks up. Reconfigure to UTF-8 with safe fallback so log
# lines never crash the audio pipeline.
